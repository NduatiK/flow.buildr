        ...
        Leaf leaf ->
            ...
            if newSize >= maxSize then
                let
                    initial =
                        Node
                            { boundingBox = BoundingBox2d.aggregate leaf.boundingBox [ BoundingBox2d.singleton (toPoint vertex) ]
                            , ne = Empty
                            , se = Empty
                            , nw = Empty
                            , sw = Empty
        ...
        Node node ->
            ...
            if BoundingBox2d.contains point node.boundingBox then
                ...
                case quadrant node.boundingBox point of
                    NE ->
                        Node
                            { boundingBox = BoundingBox2d.fromExtrema { minX = minX, maxX = Quantity.sum [ maxX, width ], minY = minY, maxY = Quantity.sum [ maxY, height ] }
                    ...
                    SE ->
                        Node
                            { boundingBox = BoundingBox2d.fromExtrema { maxY = maxY, minX = minX, maxX = Quantity.sum [ maxX, width ], minY = Quantity.minus height minY }
                    ...
                    NW ->
                        Node
                            { boundingBox = BoundingBox2d.fromExtrema { maxX = maxX, minY = minY, maxY = Quantity.sum [ maxY, height ], minX = Quantity.minus width minX }
                    ...
                    SW ->
                        Node
                            { boundingBox = BoundingBox2d.fromExtrema { maxX = maxX, maxY = maxY, minX = Quantity.minus width minX, minY = Quantity.minus height minY }
    ...
    let
        ( midX, midY ) =
            BoundingBox2d.centerPoint boundingBox
                |> Point2d.toTuple Quantity.unwrap

        ( x, y ) =
            Point2d.toTuple Quantity.unwrap point
    in
    if y >= midY then
        if x >= midX then
            NE
